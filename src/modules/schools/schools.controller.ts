...
export class SchoolsController {
  ...
  async subscribeSchool(
    ...
    const existedSubscribe = await this.schoolsService.getSubscribedSchool(user.id ,param.schoolId);
    if (existedSubscribe) {
      throw new HttpException('already subscribe this school', HttpStatus.BAD_REQUEST);
    }

    if(
      !(await this.schoolsService.subscribeSchool(user.id ,param.schoolId))
    ) {
  ...
  async createSchoolPage(
    ...
    if (existedSchool) {
      throw new HttpException('already existed school page', HttpStatus.BAD_REQUEST);
    }

    if(
      !(await this.schoolsService.createSchool(body.region, body.name, admin.id))
    ) {
      throw new HttpException('failed to save school page', HttpStatus.CONFLICT);
    }
